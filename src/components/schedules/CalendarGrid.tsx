                    ...
                    <>
                        ...
                        {DAYS.map((day, dayIdx) => {
                            const shift = shifts.find(s => {
                                if (s.dayIdx !== dayIdx || s.timeIdx !== timeIdx) return false;
                                
                                // Pengecekan Cuti (Leave Request)
                                const doctor = (s as any).doctorRel;
                                if (doctor && doctor.leaveRequests) {
                                    // Kita butuh tanggal spesifik untuk hari ini di kalender.
                                    // Karena DAYS punya properti 'date', kita asumsikan bulan/tahun saat ini.
                                    const now = new Date();
                                    const checkDate = new Date(now.getFullYear(), now.getMonth(), day.date);
