...
export function MobileMenu() {
  ...
  const renderSubItem = (item: any) => {
    ...
    const linkClassName = cn(
      ...
      isActive
        ...
        : "bg-[#1A1E2B] text-zinc-300 border-[#2B3145] hover:bg-[#222738]"
    );
      ...
      <>
        <div className="flex items-center gap-3">
          <div className={cn(
            ...
            isActive ? "bg-white/20 text-white" : "bg-[#131620] border border-[#2B3145] text-blue-400"
          )}>
            <item.icon className="h-4 w-4" />
          </div>
          <span>{item.name}</span>
        </div>
        <ChevronRight className={cn("h-4 w-4", isActive ? "text-white/70" : "text-zinc-500")} />
      </>
    );
    ...
    return (
      <div key={folder.id} className="mb-4">
        <button 
          ...
          className="w-full flex items-center justify-between px-2 pb-2.5 mb-2 border-b border-[#1E2230]"
        >
  ...
  return (
    <div className="lg:hidden fixed inset-0 z-[150] flex flex-col justify-end">
      ...
      {/* Bottom Sheet */}
      <div className={cn(
        "relative w-full bg-[#131620] border-t border-[#232736] rounded-t-[28px] shadow-2xl flex flex-col h-[85vh] transition-transform duration-300 ease-out",
        isClosing ? "translate-y-full" : "translate-y-0"
      )}>
        {/* Grab Handle */}
        <div className="flex justify-center pt-4 pb-2 shrink-0 cursor-ns-resize" onClick={closeMenu}>
            <div className="w-12 h-1.5 bg-[#232736] rounded-full" />
        </div>

        {/* Header Title */}
        <div className="px-6 pb-4 shrink-0 flex items-center justify-between border-b border-[#1E2230]">
            <div className="flex items-center gap-3">
                <div className="h-10 w-10 rounded-xl bg-blue-600 border border-blue-400/30 flex items-center justify-center text-white font-bold shadow-sm shrink-0">
                    <Activity size={20} />
                </div>
                <div>
                    <h1 className="text-base font-black tracking-tight text-zinc-100 leading-none">Menu Utama</h1>
                    <p className="text-[11px] text-zinc-400 font-medium mt-1">Navigasi Sistem MedCore</p>
                </div>
            </div>
            <button 
                onClick={closeMenu}
                className="p-2 bg-[#1A1E2B] border border-[#2B3145] hover:bg-[#23283A] rounded-full text-zinc-400 transition-colors"
            >
                <X size={18} />
            </button>
        </div>
        ...
        <div className="p-5 shrink-0 bg-[#131620] border-t border-[#1E2230] absolute bottom-0 left-0 right-0">
            <div className="flex bg-[#1A1E2B] border border-[#2B3145] p-3 rounded-[18px] items-center justify-between">
                <div className="flex items-center gap-3 min-w-0 px-1">
                    <div className="h-9 w-9 rounded-full bg-[#23283A] border border-[#353C54] flex-shrink-0 flex items-center justify-center text-zinc-100 font-bold text-sm">
                        {user?.name?.charAt(0)?.toUpperCase() || "?"}
                    </div>
                    <div className="min-w-0 pr-2">
                        <p className="text-sm font-bold text-zinc-100 truncate leading-tight">{user?.name || "Memuat..."}</p>
                        <p className="text-[10px] text-blue-400 font-bold uppercase tracking-wider truncate mt-0.5">{user?.roleName || ""}</p>
                    </div>
                </div>
                <button 
                    onClick={handleLogout} 
                    className="p-2.5 bg-[#23283A] border border-[#353C54] text-rose-400 hover:bg-rose-950/40 rounded-xl transition-all flex-shrink-0 flex items-center gap-2 pr-3 font-bold text-xs"
                >
