...
export async function withMutationRateLimit(req: Request, resource: string) {
  const ip = req.headers.get('x-forwarded-for') || 'unknown';
  const allowed = await checkRateLimit(`${resource}_${ip}`, 30, 60 * 1000);
  if (!allowed) {
...
export function apiResponse(data: any, init?: ResponseInit) {
