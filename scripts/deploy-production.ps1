...
# ── 2. PRE-DEPLOY AUTOMATED DATABASE BACKUP (WAJIB & AMAN) ────────────────────
Write-Host "🛡️  [1/6] Menjalankan Backup Otomatis Database (Safe-Guard)..." -ForegroundColor Yellow
...
try {
    ...
    Write-Host "     ✅ Snapshot data JSON berhasil dibuat." -ForegroundColor Green
} catch {
    Write-Host "     ⚠️ Peringatan export JSON: $_. Melanjutkan ke pg_dump..." -ForegroundColor Yellow
}
...
try {
    ...
    if ($LASTEXITCODE -eq 0) {
        Write-Host "     ✅ PostgreSQL dump berhasil: $SqlBackup" -ForegroundColor Green
    }
} catch {
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Gagal git pull. Membatalkan deploy." -ForegroundColor Red
    exit 1
}
...
npm run build
if ($LASTEXITCODE -ne 0) {
    Write-Host "❌ Build gagal! Server tetap berjalan dengan versi sebelumnya." -ForegroundColor Red
    exit 1
}
...
if ($LASTEXITCODE -ne 0) {
    Write-Host "⚠️ PM2 reload mengalami kendala, mencoba start ecosystem..." -ForegroundColor Yellow
