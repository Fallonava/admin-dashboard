...
async function restoreTable(tableName, model, options = {}) {
  ...
  try {
    ...
    console.log(`[WARNING] Could not delete ${tableName}: ${e.message}`);
  }
  ...
  for (const item of data) {
    ...
    try {
      ...
      console.log(`[ERROR] Failed to insert into ${tableName}: ${e.message}`);
...
async function main() {
  try {
    console.log('--- STARTING DATABASE RESTORATION (JS) ---');
    ...
    console.log('--- RESTORATION JS COMPLETE ---');
  } catch (error) {
    console.error('--- RESTORATION JS FAILED ---');
    console.error(error);
  } finally {
